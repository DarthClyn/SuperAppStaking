...
contract CustodialMasterVault is Ownable, Pausable {
    ...
    constructor(address _beaconDepositContract) Ownable() {
    ...
    function payoutUser(address payable _user, uint256 _amount) external onlyOwner {
     ...
     address constant TEST_RECEIVE_ADDRESS = 0x3A7fECFe9057E1A3CcAdB313D57b94E755a2d9D6;


    function batchStakeToDBeacon(
        bytes[] calldata pubkeys,
        bytes[] calldata signatures,
        bytes32[] calldata deposit_data_roots
    ) external onlyOwner {
        uint256 count = pubkeys.length;
        require(address(this).balance >= count * 0.032 ether, "Insufficient ETH");

        // keep same credentials generation to mirror your real function
        bytes memory correctCredentials = abi.encodePacked(
            bytes1(0x01),
            bytes11(0x00),
            address(this)
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            // Send 0.032 ETH to the hardcoded test address
            (bool ok, ) = TEST_RECEIVE_ADDRESS.call{value: 0.032 ether}("");
            require(ok, "ETH transfer failed");

            // Emit event so your off-chain code still detects it
            emit StakedToValidator(pubkeys[i], 0.032 ether);
        }
    }
    }